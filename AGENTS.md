...
This app is **Eval Vault**: it runs LLM evals against N Claude models and
archives every input, output, score, and trace to Backblaze B2 under
`evals/runs/<run_id>/`. The eval engine lives in `services/api/app/service/`
(`eval_runner`, `scoring`, `eval_store`, `comparison`); the Anthropic SDK is
contained to `services/api/app/repo/provider_anthropic.py`.
...
evals/             Shipped example eval-definition YAMLs (repo root)
...
**Eval Vault screens (built on the kit)**
- **Dashboard (`/`)** — the Eval Scoreboard: evals defined, runs archived, artifacts stored, avg score, a score-trend chart, and a recent-runs table.
- **Evals (`/evals`)** — list eval definitions and run them against the configured Claude targets.
- **Library (`/library`)** — scoped explorer over `evals/runs/`; drill into a run for the cases × targets score grid and the input/output/score/trace viewer.
- **Compare (`/compare`)** — diff two runs to hunt regressions.
- New aggregations flow through the same `runtime -> service -> repo` layering and are exposed via TanStack Query hooks in `apps/web/src/lib/queries.ts` — no bare `useEffect + fetch`.

**Why the kept pieces stay**
- The UI kit, Files (full bucket explorer), and Upload pages are the reusable B2-backed scaffolding. The Library is a *scoped* explorer that sits alongside — not replacing — the full bucket explorer at `/files`.
...
- No `boto3` outside `repo/` (B2 S3 client lives in `repo/b2_client.py`)
- No `anthropic` outside `repo/` (Claude provider adapter lives in `repo/provider_anthropic.py`)
...
| No anthropic outside repo/ | `tests/test_structure.py::test_anthropic_only_in_repo` |
