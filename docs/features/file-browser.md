...
## Purpose
List, preview, download, and delete every object stored in Backblaze B2 — including the archived eval runs under `evals/runs/`. This is the full-bucket explorer; the scoped run explorer lives at `/library` (see [Eval Library](eval-library.md)).
...
- Preview: opens dialog, fetches a preview-only presigned URL via `/files/{key}/preview` (does not count as a download)
