...
- E2E: `pnpm test:e2e` (run `pnpm --filter @eval-vault/web exec playwright install chromium` once first)
...
External SDKs live only in the `repo/` layer, verified by structural tests:
- `boto3` → `repo/b2_client.py`
- `anthropic` → `repo/provider_anthropic.py`

The service layer talks to Claude through the small `Provider` protocol, so a
different chat provider could be added as another model-under-test without
touching the eval engine.
