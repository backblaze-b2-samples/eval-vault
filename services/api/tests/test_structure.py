...
def test_anthropic_only_in_repo():
    """Verify the anthropic SDK is only imported in app/repo/.

    Mirrors test_boto3_only_in_repo: the Claude provider adapter
    (repo/provider_anthropic.py) is the only place `anthropic` may appear,
    so a different chat provider can be slotted in without touching the
    service/runtime layers.
    """
    violations = []
    for layer in LAYER_ORDER:
        if layer == "repo":
        ...
        for pyfile in _get_python_files(layer_dir):
            for imp in _get_imports(pyfile):
                if imp == "anthropic" or imp.startswith("anthropic."):
                    rel = pyfile.relative_to(APP_ROOT.parent)
                    violations.append(f"{rel}: anthropic imported outside repo/")
    assert violations == [], "anthropic boundary violations:\n" + "\n".join(violations)
