...
def get_s3_client():
    return boto3.client(
        ...
        config=Config(
            ...
            user_agent_extra="b2ai-eval-vault",
        ),
    )
...
# --- JSON artifact helpers (eval run bundles) ---
# These reuse the single UA-compliant S3 client above. Every eval input,
# output, score, and trace is archived as a small immutable JSON object.


def put_json(key: str, obj: dict) -> None:
    """Archive a JSON object to B2. Raises RuntimeError on S3 failure."""
    client = get_s3_client()
...
def get_json(key: str) -> dict:
    """Read a JSON artifact from B2. Raises RuntimeError on S3 failure."""
    client = get_s3_client()
    try:
...
def list_prefix(prefix: str, max_keys: int = 1000) -> list[str]:
    """Return every object key under a prefix, paginating as needed.
    ...
    try:
        ...
        raise RuntimeError(f"B2 list_prefix failed for '{prefix}': {e}") from e
    return keys
